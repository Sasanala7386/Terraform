...
USER_ARN=$(aws iam get-user --user-name "$USER_NAME" | jq -r '.User.Arn')
check_exit_status "Failed to retrieve IAM user ARN."
...
# Create S3 Bucket
S3_BUCKET_NAME="tf-backend-museelver"
...
sed -e "s|RESOURCE|arn:aws:s3:::$S3_BUCKET_NAME|" \
    -e "s|KEY|terraform.tfstate|" \
    -e "s|ARN|$USER_ARN|" "$(dirname "$0")/s3_policy.json" > new-policy.json
